...
<div>
    ...
    <div class="card shadow-sm p-4 mb-4" style="border-radius: 18px;">
        <div id="questions-table-container">
            @include('select.teacher.table.questions_table')
        </div>
    </div>
</div>
      ...
      <div class="modal-body row">
        ...
        <div class="col-md-4 mb-3">
            <div class="mb-3">
                ...
                <select class="form-select" id="addTopicSelect">
                    {{-- Diisi dinamis oleh JS --}}
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Subtopik</label>
                <select class="form-select" name="topic_detail_id" id="addSubtopicSelect">
                    {{-- Diisi dinamis oleh JS --}}
                </select>
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Type</label>
                <select class="form-select" name="type" id="addTypeSelect">
            ...
            <div class="mb-3">
                ...
                <input type="number" class="form-control" name="answer_number" id="addQuestionNumberInput" min="1" required>
                <small class="text-muted">Nomor soal untuk type yang dipilih (otomatis ke nomor berikutnya, bisa diubah)</small>
            </div>
            <div class="mb-3">
    ...
    <form id="questionForm" class="modal-content">
      ...
      <div class="modal-body row">
        ...
        <div class="col-md-4 mb-3">
            ...
            <div class="mb-3">
                <label class="form-label fw-bold">Type</label>
                <input type="text" class="form-control" id="modalTypeDisplay" readonly>
                <input type="hidden" name="type" id="modalTypeHidden">
            </div>
            <div class="mb-3">
                <label class="form-label fw-bold">Question Number</label>
...
function renderTable() {
    ...
    if (!subtopic) {
        tbody = `<tr><td colspan="5" class="text-center text-muted">No subtopic selected.</td></tr>`;
    } else {
        const questions = window.allQuestions.filter(q => q.topic_detail_id == subtopic.id);
        const percobaans = questions.filter(q => q.type === 'percobaan').sort((a,b) => a.answer_number - b.answer_number);
        const tugas = questions.filter(q => q.type === 'tugas').sort((a,b) => a.answer_number - b.answer_number);

        const maxPercobaan = subtopic.total_percobaan || 0;
        const maxTugas = subtopic.total_tugas || 0;

        if (maxPercobaan === 0 && maxTugas === 0 && questions.length === 0) {
            tbody = `<tr><td colspan="5" class="text-center text-muted">No questions found. Click "Add Question" to start.</td></tr>`;
        } else {
            // Section Percobaan
            if (maxPercobaan > 0 || percobaans.length > 0) {
                const totalRows = Math.max(maxPercobaan, percobaans.length);
                tbody += `<tr><td colspan="5" class="fw-bold text-center" style="background:#fff3cd; color:#856404;">Percobaan</td></tr>`;
                for (let i = 1; i <= totalRows; i++) {
                    let q = percobaans.find(q => q.answer_number == i);
                    tbody += `<tr>
                        <td class="text-center">Percobaan ${i}</td>
                        <td>${q ? q.expected_query : '<span class="text-muted fst-italic">No SQL Query</span>'}</td>
                        <td class="text-center">${q ? (q.expected_table ?? '<span class="text-muted fst-italic">-</span>') : '<span class="text-muted fst-italic">-</span>'}</td>
                        <td class="text-center">
                            <button class="btn btn-sm btn-warning" title="${q ? 'Edit' : 'Add'}"
                                onclick="editQuestion(${q ? q.id : 'null'}, ${subtopic.id}, ${i}, 'percobaan')">
                                <i class="fas fa-edit"></i>
                            </button>
                            ${q ? `<button class="btn btn-sm btn-danger ms-1 delete-question-btn" data-id="${q.id}" title="Delete"><i class="fas fa-trash"></i></button>` : ''}
                        </td>
                    </tr>`;
                }
            }
            ...
            // Section Tugas
            if (maxTugas > 0 || tugas.length > 0) {
                const totalRows = Math.max(maxTugas, tugas.length);
                tbody += `<tr><td colspan="5" class="fw-bold text-center" style="background:#cfe2ff; color:#084298;">Tugas</td></tr>`;
                for (let i = 1; i <= totalRows; i++) {
                    let q = tugas.find(q => q.answer_number == i);
                    tbody += `<tr>
                        <td class="text-center">Tugas ${i}</td>
                        <td>${q ? q.expected_query : '<span class="text-muted fst-italic">No SQL Query</span>'}</td>
                        <td class="text-center">${q ? (q.expected_table ?? '<span class="text-muted fst-italic">-</span>') : '<span class="text-muted fst-italic">-</span>'}</td>
                        <td class="text-center">
                            <button class="btn btn-sm btn-warning" title="${q ? 'Edit' : 'Add'}"
                                onclick="editQuestion(${q ? q.id : 'null'}, ${subtopic.id}, ${i}, 'tugas')">
                                <i class="fas fa-edit"></i>
                            </button>
                            ${q ? `<button class="btn btn-sm btn-danger ms-1 delete-question-btn" data-id="${q.id}" title="Delete"><i class="fas fa-trash"></i></button>` : ''}
                        </td>
...
function reloadQuestionsTable() {
    $.ajax({
        url: '/select/teacher/questions/list',
        method: 'GET',
        cache: false,
...
function reloadSubtopics(callback) {
    $.ajax({
        url: '/select/teacher/subtopics/list', // Pastikan endpoint ini mengembalikan array subtopics terbaru
        method: 'GET',
        cache: false,
        success: function(data) {
...
function reloadAllQuestionsData(callback) {
    $.when(
        $.get('/select/teacher/topics/list'),
        $.get('/select/teacher/subtopics/list'),
        $.get('/select/teacher/questions/list')
...
$('#addQuestionForm').off('submit').on('submit', function(e) {
    ...
    $.ajax({
        url: '/select/teacher/questions/save',
        method: 'POST',
        data: formData,
...
$(document).off('click', '.delete-question-btn').on('click', '.delete-question-btn', function() {
    ...
    $.ajax({
        url: '/select/teacher/questions/delete/' + id,
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
...
$('#addQuestionBtn').off('click').on('click', function() {
    const activeTopicId = window.filterState && window.filterState.topic ? window.filterState.topic : (window.topics[0] ? window.topics[0].id : null);

    $('#addQuestionForm')[0].reset();
    $('#addModulePreview').html('<span class="text-muted">Pilih subtopik untuk melihat modul.</span>');

    // Isi dropdown topik
    let topicOptions = '';
    window.topics.forEach(function(t) {
        const selected = String(t.id) === String(activeTopicId) ? 'selected' : '';
        topicOptions += `<option value="${t.id}" ${selected}>${t.title}</option>`;
    });
    $('#addTopicSelect').html(topicOptions);

    // Fungsi untuk memperbarui dropdown subtopik berdasarkan topik yang dipilih
    function refreshSubtopicOptions(topicId) {
        const relatedSubtopics = window.subtopics.filter(st => String(st.topic_id) === String(topicId));
        let subtopicOptions = '';
        relatedSubtopics.forEach(function(st) {
            subtopicOptions += `<option value="${st.id}">${st.title}</option>`;
        });
        $('#addSubtopicSelect').html(subtopicOptions);
    }

    // Fungsi update preview PDF dan nomor soal saat subtopik / type berubah
    function refreshAddModal() {
        const selectedSubtopicId = $('#addSubtopicSelect').val();
        const selectedType = $('#addTypeSelect').val();
        const subtopic = window.subtopics.find(st => String(st.id) === String(selectedSubtopicId));

        // Update PDF preview
        if (subtopic && subtopic.file_path && subtopic.file_name) {
            $('#addModulePreview').html(
                `<iframe src="/${subtopic.file_path}${subtopic.file_name}" style="width:100%;height:100%;min-height:300px;border:none;border-radius:8px;"></iframe>`
            );
        } else {
        ...
        // Update nomor soal berikutnya
        if (subtopic) {
            const existingForType = window.allQuestions.filter(q =>
                String(q.topic_detail_id) === String(subtopic.id) && q.type === selectedType
            );
            const maxNum = existingForType.length > 0 ? Math.max(...existingForType.map(q => q.answer_number)) : 0;
            $('#addQuestionNumberInput').val(maxNum + 1);
        }
    }

    // Jalankan saat pertama kali modal dibuka
    refreshSubtopicOptions(activeTopicId);
    refreshAddModal();

    // Re-run subtopik & preview saat topik berubah
    $('#addTopicSelect').off('change').on('change', function() {
        refreshSubtopicOptions($(this).val());
        refreshAddModal();
    });

    // Re-run saat subtopik atau type berubah
    $('#addSubtopicSelect').off('change').on('change', refreshAddModal);
    $('#addTypeSelect').off('change').on('change', refreshAddModal);

    $('#addQuestionModal').modal('show');
});

// Handler edit question button click
window.editQuestion = function(questionId, topicDetailId, answerNumber, type) {
    $('#questionForm')[0].reset();
    $('#topicDetailIdInput').val(topicDetailId);
    $('#answerNumberInput').val(answerNumber);
    $('#modalTypeHidden').val(type);
    $('#modalTypeDisplay').val(type ? (type.charAt(0).toUpperCase() + type.slice(1)) : '-');
    ...
    $('#modalQuestionNumber').val(answerNumber ? (type ? type.charAt(0).toUpperCase() + type.slice(1) + ' ' + answerNumber : 'Question ' + answerNumber) : '-');
...
function updateAddQuestionBtnState() {
    if (window.topics && window.topics.length > 0) {
        ...
        $('#addQuestionBtn').prop('disabled', true).attr('title', 'Please add a topic first');
    }
}
